...
load('/home/jij2009/eqtl_multi_method/pseudo_eqtl/related_genes.RData')
load('/home/jij2009/eqtl_multi_method/pseudo_eqtl/entrez2genome.RData')
load('/home/sas2030/all_human_blat_matches.RData')
...
#' coordinates.
find.pseudo.cis <- function(gene, chrom, min, max=min, cis.threshold=1, report=FALSE) {  
  ...
  # If we have multiple loci, deal with one at a time
  if ( length(gene) > 1 )
    return ( sapply(1:length(gene), function (i) find.pseudo.cis(gene[i], chrom[i],
                                                                min[i], max[i],
                                                                cis.threshold=cis.threshold,
                                                                report=report)) )
  
  idx <- match(gene, entrez2genome$entrez)
  partners <- with(related.genes, partner[id==gene])
  
  # If there are no related genes, abort
  if ( length(partners) < 1 )
    return ( -2 )
  
  # Recursively collect partners of partners (excluding geneitself)
  plen <- 0
  while ( plen < (plen <- length(partners)) ) 
    partners <- setdiff(union(partners, with(related.genes, partner[id %in% partners])), gene)
...
detect_pseudo_trans <- function(eqtl.info, entrez2genome){

    e2g.idx <- match(eqtl.info$pheno_symbol, entrez2genome$symbol)
#    eqtl.info$symbol <- entrez2genome$symbol[e2g.idx]

    # Calculate distance for all eqtl loci
    eqtl.info$distance <- with(eqtl.info, find.distance.idx(e2g.idx, geno_chr, geno_pos))
    eqtl.info$entrez <- entrez2genome$entrez[e2g.idx]
    ...
    pseudo.results <-with(eqtl.info[trans.idx, ],
                          find.pseudo.cis(entrez, geno_chr, geno_pos, report=TRUE))
    ...
    eqtl.info$cis.gene <- NA
    eqtl.info$cis.gene[trans.idx] <- pseudo.results
    eqtl.info$pseudo.dist <- NA
    eqtl.info$pseudo.dist[trans.idx] <- with(eqtl.info[trans.idx,], find.distance(cis.gene, geno_chr, geno_pos))

    pseudo.results.blat <-with(eqtl.info[trans.idx, ],
                               find.blat.pseudo.cis(entrez, geno_chr, geno_pos, report=TRUE))
    
    # If pseudo.results is a postive number, we have a pseudo cis hit
    eqtl.info$pseudo.cis.blat <- FALSE
    eqtl.info$pseudo.cis.blat[trans.idx] <- pseudo.results.blat > 0
    eqtl.info$cis.gene.blat <- NA
    eqtl.info$cis.gene.blat[trans.idx] <- pseudo.results.blat

    eqtl.info$pseudo.cis.final <- (eqtl.info$pseudo.cis | eqtl.info$pseudo.cis.blat)
#   pseudo.blat.eqtl <- subset(eqtl.info, pseudo.cis)
#    blat_pseudo_idx <- eqtl.info$pseudo.cis.blat
#    eqtl.info$m.idx[blat_pseudo_idx] <- pseudo.results[pseudo.results > 0]
#    eqtl.info$pseudo.dist[blat_pseudo_idx] <- with(eqtl.info[pseudo.blat.eqtl,], find.match.distance.idx(m.idx, geno_chr, geno_pos))


#  eqtl.info$pct.score[blat_pseudo_idx] <- eqtl.info$pct.score[eqtl.info$m.idx[blat_pseudo_idx]]
#    eqtl.info$pct.query[blat_pseudo_idx] <- eqtl.info$pct.query[eqtl.info$m.idx[blat_pseudo_idx]]
