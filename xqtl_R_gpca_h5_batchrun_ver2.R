...
PREFIX <- paste(PREFIX, "_" , method, "_Rgpc", n_gpcs, sep = "")
...
if(method == "LINEAR"){
        ...
        if(n_gpcs > 0){
            message('- Including Genotype PCs \n')
            pvals_list <- glmApply(phenotypes ~ SNP + geno_pc_matrix[,c(1:n_gpcs)],
                                   features=genotypes,
                                   nthreads=MC.CORES)$pValues

        } else {
            message('- Linear Regression without Genotype PCs \n')
            pvals_list <- glmApply(phenotypes ~ SNP,
                                    features=genotypes,
                                    nthreads=MC.CORES)$pValues
    ...
    if(n_gpcs > 0){
        ...
        message('- Linear Mixed Model Fitting without Genotype PCs \n')
        pvals_list <- parallel::mclapply(1:n.phenotypes, function(x) 
